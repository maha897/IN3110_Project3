...
# I added this test
@pytest.mark.parametrize(
    "year, downhill, beavercreak, meribel",
    [
        ("2022-23", 15, 3, 3),
        ("2021-22", 13, 4, 2),
        ("2020-21", 12, 0, 0),
...
def test_time_plan(year, downhill, beavercreak,meribel):
    url = f"https://en.wikipedia.org/wiki/{year}_FIS_Alpine_Ski_World_Cup"
    markdown = time_plan(url)
    assert markdown.count("Downhill") == downhill
    assert markdown.count("Sölden") == 1
    assert markdown.count("Beaver Creek") == beavercreak
    assert markdown.count("Méribel") == meribel